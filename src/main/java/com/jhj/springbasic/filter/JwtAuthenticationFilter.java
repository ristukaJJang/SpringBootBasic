package com.jhj.springbasic.filter;
...
// filter : 서버 로직과 서블릿 사이에서 http request에 대한 사전 검사 작업을 수행하는 영역
// - filter에서 걸리진 request는 서블릿까지 도달하지 못하고 reject됨
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
        ...
        throws ServletException, IOException
    {
        try{
            ...
            // - 3) empty security context 생성
            SecurityContext securityContext = SecurityContextHolder.createEmptyContext();
            
            // - 4) security context 생성한 인증 정보 토큰 등록
            securityContext.setAuthentication(authenticationToken);

            // - 5) 생성한 security context를 등록
